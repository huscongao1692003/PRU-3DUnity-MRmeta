...
public class Main : MonoBehaviour {
    ...
    void Update() {
        ...
        // Left/Right Movement (Primary Thumbstick)
        Vector2 leftStickValue = OVRInput.Get(OVRInput.Axis2D.PrimaryThumbstick);

        // Left/Right Rotation (Secondary Thumbstick)
        ...
        if (Time.timeScale != 0) {
            ...
            if (isMoving) {
                ...
                if (timeForNextCheck <= 0) {
                    ...
                    // movement
                    if (leftStickValue.x < -0.5f && canMove) {
                    ...
                    if (leftStickValue.y < -0.5f && canMove) {
                        if (!isMovePossible(currentScript.block.block, 0, 0, -1)) {
                            currentScript.z -= 1;
                            currentScript.fixPositionZ();
                            createHintBoxes();
